...
**Current release:** v0.4.6 — Cumulative Self-Healing PATCH / Git Bridge v1.2.0
**Updated:** 2026-09-20
**Service port:** `127.0.0.1:8793` by default
**SQLite schema:** v2 (unchanged)
**Upgrade base:** direct cumulative PATCH upgrade supported from v0.3.0+; intermediate patches are NOT assumed. PATCH is the primary install artifact and must preserve active config + state.
...
The exact final PATCH ZIP is a **cumulative public-source overlay**, not a minimal diff. It must contain every current canonical source/runtime/tooling file required to self-heal a machine that skipped one or more intermediate patches. It must be overlay-tested against the immediately prior field release, the oldest supported baseline, and at least one deliberately stale/mixed tree, with seeded `discovery_config.json`, SQLite/state, and unrelated private-runtime sentinels that remain byte-for-byte unchanged. The exact final FULL SOURCE ZIP must be freshly extracted and package-validated.
...
Cumulative self-healing PATCH redesign. The fourth Windows bootstrap log proved that even the new current-release validator could fail when the live tree contained stale **product code**: `validate_current.py` expected the repaired nested catalog importer to produce four unique models but observed `3/3`. That exact result was reproduced with the v0.4.0 importer. The clean v0.4.5 FULL SOURCE had the repaired importer, but v0.4.5 PATCH did not carry unchanged core files such as `discovery/importers.py`. A user who skipped v0.4.1 could therefore reach a nominal v0.4.5 tree with stale v0.4.0 runtime code.

v0.4.6 fixes the release model itself:

- every PATCH is cumulative and includes the complete current public runtime/source/tooling surface;
- `PATCH_MANIFEST.json` declares the cumulative-overlay contract and minimum supported version (`0.3.0`);
- `validation/source_manifest.json` SHA-256 pins canonical executable/validation/bridge source;
- `validation/runner.py` verifies canonical hashes before behavioral tests;
- Git Bridge v1.2.0 rejects non-cumulative patches, missing canonical files, and hash mismatches before extraction/copy;
- `tools/release/build_release.py` is the authoritative final artifact builder;
- catalog/Recu parser cursor generations bump to `v46` / `v46|` for a one-time corrective reparse after this repair.

The release was tested by applying only the v0.4.6 cumulative PATCH onto (a) v0.3.0, and (b) a deliberately poisoned v0.4.0-based tree that had later version/validation patches but retained the old importer. Both became canonical v0.4.6 trees, preserved private state byte-for-byte, passed runtime validation, produced public-safe staging that passed package validation, and the stale-tree case completed a local bare-origin Git Bridge bootstrap simulation through branch, commit, simulated PR squash merge, tag, and clean final worktree.
...
## 28. v0.4.6 fourth Git Bridge incident — stale product source exposed

### Field failure

The user's v0.4.5 bootstrap correctly used `validation/runner.py`, ignored historical validators, compiled the current tree, and passed the unit suite. It then failed the authoritative current validator at the nested-catalog regression:
...
This was not a bad current assertion. The exact `seen:3 / imported:3` behavior is produced by the old v0.4.0 `discovery/importers.py`, which treated the outer `original/review/deletion` catalog buckets as model accounts. The repaired v0.4.1+ importer yields five row/group records and four unique models for the same fixture.

### Root cause

The release pipeline still assumed every intermediate PATCH had been installed. v0.4.5 PATCH was a minimal changed-file overlay, so it contained the new version/validation/bridge files but not the unchanged repaired importer. A skipped/interrupted v0.4.1 update could therefore leave old runtime code under a newer version marker. Clean FULL SOURCE tests could not detect that mixed-tree failure.

### Permanent correction

The release invariant is now:

> **Every CTBRec Discovery PATCH is a cumulative, self-healing public-source overlay.** It must make any supported older/mixed installation canonical without requiring intermediate patches.

Mechanisms:

1. `PATCH_MANIFEST.json` declares `patch_type=cumulative_public_source_overlay`, current version, `min_supported_version=0.3.0`, and source-manifest path.
2. `validation/source_manifest.json` pins SHA-256 for canonical executable, UI, validation, Git Bridge, release-tooling, and test files.
3. `validation/runner.py` verifies canonical hashes before compile/unit/behavioral validation. A stale source file now fails immediately with a targeted source-drift message.
4. Git Bridge v1.2.0 opens the PATCH ZIP before extraction and rejects it unless every canonical source file exists and matches the manifest hash.
5. `tools/release/build_release.py` builds PATCH cumulatively from current public source. PATCH is no longer a minimal diff.
6. `docs/RELEASE_ENGINEERING_PROMPT.md` requires direct-upgrade testing from the oldest supported baseline and a deliberately stale/mixed tree.
7. Catalog and Recu local cursor generations are bumped to `v46` / `v46|` to force one corrective local reparse after the repaired code lands.

## 29. Standing delivery invariant after v0.4.6

- PATCH is always the primary artifact and is generated last.
- PATCH is always cumulative/self-healing; intermediate patch installation is never assumed.
- Exact final PATCH must include every file in `validation/source_manifest.json`.
- Exact final PATCH is tested against prior, oldest-supported, and stale/mixed upgrade shapes where artifacts are available.
- Private config/state/DB/log/session/media files must remain byte-for-byte untouched.
- FULL SOURCE + complete handoff + test report + checksums remain mandatory.
- No email delivery; Git/Git Bridge is the normal release-history path.

## 30. v0.4.6 frozen release validation

Release gates for the final artifact are documented in `TEST_REPORT_v0_4_6.txt`. Required proof includes:

1. current source `validation/runner.py --mode package`: PASS;
2. 27/27 unit/regression tests: PASS;
3. Node syntax check: PASS;
4. current nested-catalog validator sees **5 row/group records and 4 unique models**: PASS;
5. source-manifest verification: PASS;
6. cumulative PATCH ZIP contract validation: PASS; every canonical manifest file is present and hash-correct;
7. direct v0.3.0 -> v0.4.6 exact-PATCH overlay with private config/SQLite/sentinel preservation: PASS;
8. poisoned mixed tree reproducing the old v0.4.0 importer -> v0.4.6 exact-PATCH overlay: old importer replaced, private state unchanged, runtime validation PASS;
9. public-safe Git staging from both exact patched trees: package validation PASS;
10. local bare-origin Git Bridge setup from the exact patched stale/mixed tree: PASS through validation, bootstrap branch, commit, push, simulated PR squash merge, `main` refresh, `ctbrec-discovery-v0.4.6` tag, clean worktree, and external bridge config;
11. clean exact FULL SOURCE extraction + package validation: PASS;
12. read-only production GitHub check before release: previous failed setup attempts have not partially published Discovery; remote `main` still contains only the historical `test.md`.

## 31. Next engineering priorities after Git bootstrap succeeds

Once the user's real Git Bridge bootstrap passes, stop spending feature cycles on release plumbing and return to Discovery product work:

1. verify the repaired real Mobile Reviewer catalog count and Recu match diagnostics on the user's machine;
2. add Discover-card live-room playback by reusing the existing Live Control player/proxy methodology;
3. add local Recu Best Moments playback by reusing Rapid Sorter's cache/broker;
4. connect actual neighbor/Recu bookmark-momentum inputs so affinity scores become materially personalized;
5. implement the separate session-opportunity/anomaly engine;
6. preserve the three-engine separation and nonblocking architecture.
