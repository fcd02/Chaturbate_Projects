...
**Current release:** v0.4.4 — Git Bridge Incremental-Upgrade Validation Hotfix
**Updated:** 2026-09-20
**Service port:** `127.0.0.1:8793` by default
**SQLite schema:** v2 (unchanged)
**Upgrade base:** v0.4.3 (and intentionally self-heals missing historical validator files from any prior incremental patch path); patch is source/docs/tests/tooling only and must preserve active config + state.
...
5. email to `christianpwire@gmail.com` by replying in the existing Gmail thread **CTBRec Discovery Build Updates**.

Never ship active `discovery_config.json`, SQLite/runtime state, logs, cookies, Recu session/browser-profile material, Chaturbate WM IDs, PINs/passwords/secrets, or other user-private runtime state.

Gmail blocks archives containing script types and also rejected the previous base64 text transport. v0.4 used Base85 plain-text representations of the exact ZIP bytes as the email fallback. Normal ZIPs remain the primary ChatGPT artifacts.
