# CTBRec Mobile Reviewer v2.15.10 — Complete Development Handoff

**Release type:** Git Bridge Windows PowerShell compatibility hotfix; Reviewer feature behavior unchanged from v2.15.9  
**Current server identity:** `CTBRecMobile/2.15.10`  
**PWA shell:** `ctbrec-shell-v21510`  
**Static cache query:** `?v=21510`

## Why v2.15.10 exists

The first real Windows field run of `Setup-Git-Bridge.cmd` authenticated successfully, fast-forwarded `fcd02/Chaturbate_Projects`, then failed before copying any source:

`Cannot convert argument "trimChars", with value: "\\", for "TrimStart" to type "System.Char".`

PowerShell uses the backtick, not backslash, as its escape character. The v2.15.8/v2.15.9 bridge therefore passed the two-character string `\\` to a .NET `TrimStart(Char[])` overload. GitHub was checked after the failure and still contained only the user's `test.md`; no source was partially published and the live Reviewer was not touched.

## v2.15.10 implementation

### Windows path handling
- Added `Get-RelativeChildPath()` to both Setup and Apply bridge scripts.
- Child paths now use regex separator stripping (`-replace '^[\\/]+',''`) rather than `TrimStart('\\','/')`.
- The same broken pattern was removed from both patch-copy and runtime-backup paths before the user could hit it on the next update.
- Runtime-vs-repo equality no longer uses `TrimEnd('\\')`; `Normalize-ComparePath()` performs separator-safe comparison.

### Python resolver audit
A second latent Windows/PowerShell bug was found during the same audit. `Get-PythonCommand` returned a one-element array such as `@('py')`, while callers used `(Get-PythonCommand)[0]`. PowerShell may unwrap the one-element return to a scalar string, making `[0]` the character `p`. v2.15.10 returns the resolved executable path as a scalar string and callers invoke it directly.

### Git first-run resilience
- Added `Ensure-GitIdentity()` to Setup and Apply.
- If `user.name`/`user.email` are missing, the bridge derives the authenticated GitHub login/id via `gh api user` and sets repository-local identity only, using GitHub's noreply address format.
- `.git` internals and `git_bridge.local.json` are explicitly excluded from public mirroring.
- `models/README.md` is now permitted through the public filter, while ONNX/model binaries remain blocked.

## Validation

New `validation/validate_v2_15_10_git_bridge_windows_compat.py` locks the field regression and the additional audit fixes. It verifies the invalid TrimStart/TrimEnd signatures cannot return, the Python resolver is scalar-safe, path comparison is normalized, Git identity fallback exists, public-source guards remain active, and CMD launchers still use process-local `-ExecutionPolicy Bypass`.

Final full-suite / packaged ZIP / patch-overlay results are appended below after release packaging.


## Final release-package validation

Final packaging gate for v2.15.10:

- Working source: Python compileall + Node syntax checks + **31/31 validation suites PASS**.
- Actual FULL SOURCE ZIP: extracted and checked with Python compileall and `node --check` for app/rapid/service-worker; the complete **31-suite regression set PASS** in two explicit batches (16 + 15) to avoid the harness wall-clock timeout.
- Actual PATCH ZIP overlay: seeded a v2.15.9 installation with **15 protected runtime/private sentinels** (config, roots, action/catalog/READY/library/NSFW/model/hidden/Recu state, Recu browser state and ONNX binary); after overlay, **15/15 SHA-256 values remained identical**.
- PATCH inventory contains **30 source/documentation files** and none of the protected runtime/private paths.
- GitHub repository was inspected after the user's failed v1.0 setup and still contained only `test.md`; the failure happened before publish.

**Environment limitation:** this release environment does not contain Windows PowerShell, so the exact `.ps1` scripts cannot be executed end-to-end here. The field-specific failure is locked by source-level regression assertions and the bridge remains fail-closed. The next real Windows run is `Setup-Git-Bridge.cmd`; if a new Windows-only failure appears, capture its console output and fix against that exact stage.

## Exact next step for the user

1. Overlay the v2.15.10 PATCH on the current v2.15.9 live folder.
2. Restart Reviewer once.
3. Re-run `tools\git_bridge\Setup-Git-Bridge.cmd`.
4. No manual Git cleanup is required from the failed run; GitHub itself was unchanged. The setup may reuse locally-created untracked umbrella docs from the failed attempt and will commit them together with the sanitized source baseline after showing the diff.

## Preserved behavior

v2.15.10 intentionally changes no Reviewer review/selection, preview, mosaic generation/reuse, READY/sidecar, queue/lease, bounded look-ahead, TS sort, Recu, Keep Last, action queue, or destructive file behavior. v2.15.9 seekable zero-reencode preview caching and fixed player geometry are preserved exactly.

---

## Prior v2.15.9 handoff follows for complete historical continuity

# CTBRec Mobile Reviewer v2.15.9 — Complete Development Handoff

**Release type:** preview performance/container + UI geometry stabilization
**Current server identity:** `CTBRecMobile/2.15.9`  
**PWA shell:** `ctbrec-shell-v2159`  
**Static cache query:** `?v=2159`

## Why v2.15.9 exists

The user supplied two one-click Preview Diagnostics bundles from different recording batches of the same model. They demonstrated that the perceived speed difference is not model-specific and is not primarily codec complexity:

- **Smooth:** `forthoseinattendance_2026-07-13_22-17-40_961.ts_tail_13m32s_est.mp4` is a true MP4 container with H.264 Main + AAC. Chromium accepted original-file direct playback. Seek requests became ordinary HTTP 206 byte ranges and completed in roughly 50–160 ms in the captured interaction.
- **Slow:** `forthoseinattendance_2026-03-16_15-44-43_613_tail_11m44s_est.ts` is true MPEG-TS, also H.264 Main + AAC. Chromium immediately returned media code 4 / `DEMUXER_ERROR_COULD_NOT_OPEN`. The old live stream-copy fragmented-MP4 remux then produced only ~0.194 s for a ~750.419 s recording. Compatibility transcoding worked, but each source-timeline seek launched a new FFmpeg process and needed roughly 2–3 s before `loadedmetadata`/playback.

Because the bad TS already contains browser-friendly H.264/AAC, transcoding is unnecessary. The right optimization is a one-time container-only remux to a materialized seekable MP4, followed by ordinary Range playback.

## v2.15.9 implementation

### Seekable zero-reencode cache

- New temp cache: `PREVIEW_SEEKABLE_CACHE_DIR = <system temp>/ctbrec_mobile_preview_seekable`.
- `preview_seekable_copy(path, diag_id)` creates `<source-signature>.mp4` with FFmpeg `-c copy`; video/audio are never re-encoded.
- Cache key reuses `_preview_hls_key`: normalized path + current size + mtime. A modified/replaced source cannot reuse an old wrapper.
- Build uses a unique temporary `.tmp.mp4` and `os.replace()` only after validation, so an interrupted process cannot leave a seemingly valid cache file.
- Cache output must be >64 KiB. When source duration is known and >2 s, cached duration must be at least 90% of source duration. This directly prevents recurrence of the field failure where a ~0.19 s remux was accepted for a ~750 s source.
- Cache hit updates file mtime to support LRU-like pruning.
- Cache pruning uses `video_preview.cache_hours` and new `video_preview.seekable_cache_max_gb` (default 8 GB; minimum enforcement 512 MiB). Only derivative temp MP4s are deleted; source media is never touched.
- New diagnostics events: `seekable_cache_build_start`, `seekable_cache_build_complete`, `seekable_cache_build_failed`, `seekable_cache_hit`, and `seekable_cache_http_error`.
- `?seekable=1` on the existing `/api/video/...` route materializes/reuses the cached MP4, then serves it with the existing correct `send_range_file()` HTTP Range implementation.

### Automatic ladder

- `preview_items()` now returns `seekable_url` for every source.
- Desktop direct-original is attempted only when the item is already a browser-native direct container (`.mp4`, `.m4v`, `.mov`).
- A true `.ts`/`.mpegts`/`.m2ts`/`.mts` item therefore starts at `seekable` rather than wasting a known-failing raw Chromium open.
- Current ladder: native Direct/HLS -> seekable cached MP4 -> Compatibility.
- If seekable cache creation/playback fails, Compatibility is automatic.
- Existing premature-stream-end guard remains: a backend/method failure cannot advance to the next selected recording unless the absolute source playhead is actually near source end.
- Legacy `remux` source-timeline code remains available for compatibility with older/internal paths but is no longer the preferred desktop TS path.

### Stable preview geometry

- `.preview-player-shell` now has a backend-independent responsive height (`clamp(220px, 52vh, 440px)` on normal layouts).
- `<video>` fills that viewport with `object-fit: contain`, so source resolution/aspect changes do not resize the modal.
- `#preview-note` reserves a fixed-height scrollable status area.
- Compatibility action remains present in the layout and changes disabled/text state rather than being removed with `.hidden`.
- Source-timeline controls remain an absolute overlay for nonseekable Compatibility/remux modes.
- Result: Direct -> seekable -> Compatibility transitions can change the backend source without moving the user’s scrub target or Prev/Next controls.

## Regression coverage

New `validation/validate_v2_15_9_preview_seekable_cache.py` verifies:

1. real synthetic H.264/AAC MPEG-TS -> full-duration MP4 stream-copy;
2. second request reuses exactly the cached MP4;
3. simulated successful-but-truncated remux is rejected;
4. code exposes bounded cache + diagnostics + Range endpoint;
5. real Chromium chooses `seekable` for TS/non-native and `direct` for MP4;
6. real Chromium measures constant player height and constant Prev/Next row position while switching to Compatibility/source-timeline controls.

Complete working-tree validation: **30/30 suites PASS**.

Final release-candidate package validation:

- extracted the actual `CTBRec_Mobile_Reviewer_v2_15_9_FULL_SOURCE.zip`; Python compileall, `node --check` for app/rapid/service-worker, and **30/30 validation suites PASS**;
- patch-overlay safety test seeded **15 protected runtime/private paths** (config, roots, action/catalog/READY/library/NSFW/model/hidden/Recu state, Recu browser state and ONNX sentinel); after overlaying the actual PATCH ZIP, **15/15 SHA-256 values were unchanged**;
- PATCH inventory contains none of the protected runtime/private paths;
- FULL SOURCE starter state is sanitized before packaging (blank PIN/secret, blank roots, empty catalog/action queue, empty log).

## Prompt/specification used

The exact engineering prompt used for this release is checked into `docs/prompts/PROMPT_v2_15_9_PREVIEW_SEEK_STABILITY.md`. It records the field evidence, UX contract, cache safety requirements, preservation boundaries, and required regressions.

## Preserved behavior

v2.15.9 intentionally does **not** change mosaic generation/reuse, READY/sidecar semantics, bounded active-model look-ahead, queue lifecycle/lease behavior, Review/Frame Cut decisions, symmetric Shift-range behavior, TS-only model sort, Recu, Keep Last, action queue, destructive file semantics, Git Bridge deployment behavior, or runtime/private-state layout.

---

## Prior v2.15.8 handoff follows for complete historical continuity

# CTBRec Mobile Reviewer v2.15.8 — Complete Development Handoff

**Release type:** deployment/repository tooling; Reviewer runtime behavior unchanged from v2.15.7  
**Current server identity:** `CTBRecMobile/2.15.8`  
**PWA shell:** `ctbrec-shell-v2158`  
**Static cache query:** `?v=2158`

## Why v2.15.8 exists

The user is moving project continuity/version control to the public GitHub umbrella repository `fcd02/Chaturbate_Projects`, but the ordinary ChatGPT GitHub connector available in this chat can read the repository while repository-content writes are rejected by the integration. The user also wants to keep using normal ChatGPT heavily instead of spending Codex allowance on every implementation cycle.

v2.15.8 therefore adds a permanent **CTBRec Git Bridge** that lets normal ChatGPT continue producing a validated PATCH while a local one-click Windows tool performs the mechanical Git + deployment steps with the user's already-authenticated GitHub CLI.

## Git Bridge architecture

Supported tooling lives under `tools/git_bridge/` and is now a required part of every future FULL SOURCE and PATCH package unless deliberately superseded by a documented migration.

### `Setup-Git-Bridge.cmd` / `Setup-Git-Bridge.ps1`

One-time bootstrap. Defaults:

- GitHub repository: `fcd02/Chaturbate_Projects`
- local repository: `C:\GitHub\Chaturbate_Projects`
- project folder: `projects\ctbrec-mobile-reviewer`
- exact runnable public source mirror: `projects\ctbrec-mobile-reviewer\app`
- live Reviewer: the CTBRec folder from which setup is run
- local bridge config: `%LOCALAPPDATA%\CTBRecGitBridge\config.json`

The setup tool:

1. requires Git + GitHub CLI and verifies `gh auth status`;
2. clones the umbrella repo when it is not already local;
3. fast-forwards `main`;
4. creates root README/PROJECTS/SECURITY/.gitignore plus project README/AGENTS;
5. copies a public-safe source baseline into the project `app/` tree;
6. excludes runtime state, credentials, cookies/sessions, recordings, generated mosaics, logs, Recu browser profile, model binaries and machine-private config;
7. creates canonical project `HANDOFF.md`, `CHANGELOG.md`, and release-note history;
8. persists repo/runtime paths outside Git;
9. displays the Git diff and commits/pushes only after approval.

The user's current runtime can remain in its existing folder; no migration of private runtime state is required.

### `Apply-ChatGPT-Update.cmd` / `Apply-ChatGPT-Update.ps1`

Normal future update path. The user downloads a ChatGPT PATCH ZIP and drags it onto the CMD file (or double-clicks and pastes the ZIP path).

Safety/publish/deploy sequence:

1. load local bridge config;
2. inspect the ZIP and infer `vX.Y.Z` from versioned HANDOFF/CHANGELOG;
3. reject path traversal, runtime/private files, secrets/credentials/cookies, logs, recordings/media and model binaries;
4. require a clean Git working tree;
5. checkout + fast-forward `main`;
6. create `release/ctbrec-vX.Y.Z`;
7. apply patch to the Git source mirror;
8. refresh canonical `HANDOFF.md`, `CHANGELOG.md`, release notes and root current-version index;
9. run Python compileall, Node syntax checks, and every `validation/validate_*.py`;
10. if validation fails, abort before touching the live Reviewer;
11. commit the release with the user's optional comment;
12. push the branch and create a GitHub PR whose body contains validation + release notes;
13. by default squash-merge the validated PR and tag `ctbrec-mobile-reviewer-vX.Y.Z`;
14. stop the live Reviewer using `mobile_server.pid`;
15. back up every live path the patch will replace;
16. overlay the validated patch into the live runtime;
17. run `mobile_self_test.py`;
18. restart `start_server_only.bat`;
19. poll `http://127.0.0.1:<port>/api/auth` until the new server answers;
20. if live self-test/startup fails, restore the backed-up files and restart the previous version.

If the live runtime and Git `app/` path are intentionally the same directory, the bridge detects that case and stops the Reviewer before modifying the working tree.

### Codex Windows helper

`Install-Codex-CLI.cmd` calls `npm.cmd install -g @openai/codex@latest`, specifically avoiding the PowerShell `npm.ps1` shim that failed on the user's machine because the PowerShell execution policy blocks scripts. It verifies `codex.cmd` and directs the user to `codex --login`.

The two bridge CMD launchers call PowerShell with process-local `-ExecutionPolicy Bypass`, so the user's system execution policy does not block Git Bridge operation.

`CODEX_WINDOWS_SETUP.md` also documents:

- the npm.ps1 error cause;
- optional `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned`;
- temporary `%APPDATA%\npm` PATH recovery;
- cloning `fcd02/Chaturbate_Projects`;
- where Codex should be launched for umbrella-vs-project work.

## Security / public repository rules

The umbrella repository is currently public. Public source should be as complete as practical, but actual reusable credentials should not be committed even to a private companion repository. The bridge blocks or excludes common private/runtime material including:

- `mobile_config.json`;
- `recording_roots.txt`;
- `keeplasts.txt`;
- Recu session/browser profile data;
- action queue / catalog / READY / library / hidden-model / model-metadata state;
- duration/transcript/model-size/manifest runtime caches;
- logs / PID files;
- recordings and generated media;
- ONNX/model binaries;
- `.env`, credentials, cookies, secret/key files.

The public repo should contain sanitized templates, documentation explaining required private values, and the complete source logic.

## Explicit non-changes in v2.15.8

No functional Reviewer behavior was changed beyond release/version markers. Preserve all v2.15.7 behavior, including:

- per-chunk selection reset in normal + Rapid paths;
- stale draft chunk-signature rejection;
- symmetric Shift select/unselect;
- actual-media preview duration instead of `_tail_` semantics;
- direct/remux/compatibility playback ladder and source-relative fallback timeline;
- multi-file preview staying on the same source until true source end;
- active-phone lease recovery;
- pending-open self-healing;
- bounded active-model look-ahead/refill;
- trustworthy mosaic/READY reuse;
- Random / Avg Chunk / TS-only ordering;
- Recu, Keep Last, Non-NSFW cleanup and durable action queue behavior.

## Files added/changed in v2.15.8

Added:

- `tools/git_bridge/Setup-Git-Bridge.cmd`
- `tools/git_bridge/Setup-Git-Bridge.ps1`
- `tools/git_bridge/Apply-ChatGPT-Update.cmd`
- `tools/git_bridge/Apply-ChatGPT-Update.ps1`
- `tools/git_bridge/Install-Codex-CLI.cmd`
- `tools/git_bridge/CODEX_WINDOWS_SETUP.md`
- `tools/git_bridge/README.md`
- `tools/git_bridge/git_bridge.config.example.json`
- `validation/validate_v2_15_8_git_bridge.py`
- `CHANGELOG_v2_15_8.txt`
- `UPDATE_FROM_2_15_7.txt`
- `TEST_REPORT_v2_15_8.txt`
- this current handoff

Version-marker-only updates:

- `ctbrec_mobile_server.py`
- `static/index.html`
- `static/service-worker.js`
- `mobile_self_test.py`
- inherited validators' current-release marker assertions
- `README.md`

## Validation boundary

The complete Python/JavaScript Reviewer regression suite is run in the packaging environment. The Git Bridge itself receives static regression coverage for required safety/publish/deploy/rollback invariants. The packaging environment does not provide Windows PowerShell, so the `.ps1` files cannot be end-to-end executed there. **A first-run Windows field test of Setup + one update cycle remains required.** The bridge is designed to fail closed before touching live runtime when Git validation fails and to keep rollback backups during live deployment.

## Exact next steps

1. Apply v2.15.8 to the currently working v2.15.7 installation using the existing manual patch process one final time.
2. Confirm Reviewer still starts/works (runtime logic is unchanged).
3. Run `tools\git_bridge\Setup-Git-Bridge.cmd`.
4. Approve the sanitized baseline Git diff and push.
5. Verify GitHub now contains `projects/ctbrec-mobile-reviewer/app`, canonical `HANDOFF.md`, project docs and release notes.
6. For the next CTBRec change, use the generated PATCH ZIP with `Apply-ChatGPT-Update.cmd`; validate branch/PR/merge/tag + live restart behavior in the field.
7. If Codex is desired, run `Install-Codex-CLI.cmd` or `npm.cmd install -g @openai/codex@latest`, then `codex --login`.
8. Do **not** email future release artifacts; GitHub + local bridge is the intended continuity path, while ChatGPT should continue supplying FULL SOURCE/PATCH/HANDOFF download artifacts until Git-based publication has been field-proven.

---

# Historical handoff — v2.15.7 and earlier

# CTBRec Mobile Reviewer v2.15.7 — Complete Development Handoff

**Release type:** stabilization / cross-feature regression cleanup  
**Baseline:** released v2.15.6 FULL SOURCE  
**Current server identity:** `CTBRecMobile/2.15.7`  
**PWA shell:** `ctbrec-shell-v2157`  
**Static cache query:** `?v=2157`

## Why v2.15.7 exists

The user reported three concrete problems after the v2.15.5/v2.15.6 preview and Shift-range work:

1. In Review, especially **Frame Cut**, after **Queue precise cuts & next**, selections from the prior mosaic appeared selected on the next mosaic. Selection state must be per-chunk and reset on every newly-current mosaic.
2. The existing **Delete / clear** decision label was confusing and did not provide an explicit one-click way to clear all current selections. The user requested a real Clear control.
3. Compatibility/source-timeline preview was using a `_tail_...` filename value as if it were the duration of the physical media segment. Example: a ~15-minute source segment named with `_tail_1h35m...` showed a ~1h35m scrub range, and scrubbing past 15 minutes produced black video. `_tail_` means remaining larger-session/chunk context in these CTBRec filenames, not necessarily this file's duration.
4. Follow-up requirement: Shift-click must work **symmetrically for unselecting** as well as selecting so very large review grids can be corrected in blocks.
5. User requested a broader stabilization pass after the large number of recent interacting changes: examine Rapid vs normal paths, preview/fallback interactions, queue transitions, selection state, and other cross-feature regressions rather than patching only the visible symptoms.

## Root causes found in the stabilization audit

### A. Rapid Sort had its own incomplete selection initializer

The normal `static/app.js::loadCurrent(true)` reset many per-chunk state variables, but `static/rapid.js` overrides fast navigation and calls its own `initializeSelectionsFromCurrent()`. That function did **not** reset:

- `state.frameDecisions`
- `state.lastFrameSelectionIndex`
- `state.lastFileSelectionIndex`
- `state.recoverSelected`
- frame-tile caches
- several other per-chunk UI structures

This was the primary reason Frame Cut choices could visibly survive into the next mosaic even though the server had already advanced.

v2.15.7 makes the Rapid initializer follow the same complete per-chunk reconstruction contract as the normal path. It reconstructs state only from the newly-current chunk's own draft. It intentionally preserves the user's **Frame Cut vs Whole segments granularity preference** when the new chunk has no saved granularity draft, while clearing all actual frame/file selections.

### B. Delayed autosave could cross a chunk boundary

`scheduleDraft()` used a 250ms timer and posted to `/api/queues/<queue>/draft` without identifying which chunk the payload belonged to. A rapid Submit could advance the queue before a delayed draft POST arrived; the server then stored the old chunk's choices under whatever chunk was current at receipt time.

v2.15.7 adds `_chunk_signature` to online draft/submit payloads. The server:

- rejects a draft if `_chunk_signature` does not equal the current chunk signature;
- rejects stale Submit payloads for the same reason;
- strips underscore-prefixed client metadata before persisting/action execution.

Client navigation also cancels pending draft timers where appropriate. This makes stale cross-chunk draft poisoning impossible even if requests are delayed or reordered.

### C. Original mosaic recovery discarded the exact duration already present in the sidecar

Current original mosaic `.sources.json` sidecars already contain the exact duration used when that mosaic was generated. `_recover_original_snapshot_from_mosaics()` ignored `item["duration"]` and instead reconstructed `VideoInfo.duration` from the duration cache, then `_tail_` filename parsing, then a generic rapid fallback.

That meant a perfectly good historical mosaic could be re-indexed and regain a bogus 1h35m duration solely because of its filename.

v2.15.7 now uses duration priority:

`duration cache -> validated mosaic-sidecar exact duration -> tail tag provisional -> rapid fallback`

A validated v2 sidecar therefore retains the exact duration that was used to build its frame layout.

### D. Preview source duration was not guaranteed to be media-derived

`preview_items()` previously returned `video.duration`, which may be provisional metadata from a rapid queue snapshot. v2.15.7 instead calls `preview_exact_duration(path)` for each selected preview item.

`preview_exact_duration()` now:

1. validates cache by source byte size + mtime;
2. probes **actual media** with ffprobe using both `format.duration` and video `stream.duration`;
3. falls back to parsing FFmpeg's actual input `Duration:` banner if ffprobe is absent/unhelpful;
4. does **not** use filename `_tail_` semantics for interactive preview;
5. caches only a successful actual-media duration;
6. allows a fallback only when a caller explicitly opts into `allow_fallback=True` (interactive preview/HLS does not).

If the server cannot rapidly resolve exact duration but Direct/HLS native browser metadata succeeds, the client adopts `video.duration` only for those native source-timeline stages. It never treats remux/compatibility pipe duration as source duration.

### E. Unknown duration could cause premature next-file advancement

`previewReachedSourceEnd()` used to return `finished: true` when duration was <= 0. Therefore an early-ended remux/compatibility pipe with unknown source duration could be interpreted as completion and advance to the next selected file.

v2.15.7 requires `duration > 0` before any end-of-source decision can be true. Unknown duration stays on the same file and continues the automatic playback/failure handling instead of skipping.

### F. Shift-range was one-way only

Earlier Shift support always applied selection across the range. The new behavior is symmetric and anchor-driven:

- plain click/tap keeps existing toggle semantics;
- if the most recently clicked anchor is selected, Shift-click applies selection/current destination across the inclusive range;
- if the anchor was just toggled **off**, Shift-click clears/reverts the inclusive range;
- applies to file-level Original/Review/Deletion/Cleanup selection and Frame Cut selection;
- newest click becomes the next anchor;
- anchors reset on chunk/model transitions.

This matches the user's intended Windows-like bulk correction workflow.

### G. Rapid path had silently bypassed older UI safeguards

During the audit, Rapid Sort was also found not to perform two normal-path behaviors on fast current changes:

- clear/rebuild frame-tile caches;
- scroll a newly-current mosaic back to the top.

Both are now restored in `applyOnlineCurrent()`.

Rapid's old toast text also still claimed future mosaics only generated when reached. That became stale after v2.15.2. Text now accurately says bounded automatic look-ahead maintains the configured ready buffer while the model stays open.

## Explicit Clear UX

Review now includes a dedicated button below destination choices:

- Whole segments: **Clear segment choices** -> reset every file to default `Leave for review`.
- Frame Cut: **Clear all frame selections** -> no kept frame intervals remain; unselected footage is deletion-bound per Frame Cut semantics.

The old ambiguous `Delete / clear` label is no longer pretending to be a global Clear button. In Frame Cut it displays **Delete / unselect**; in whole-segment mode it displays **DELETE**.

The Clear button also resets both Shift anchors and immediately schedules a chunk-signature-bound draft save.

## Important preserved behavior

v2.15.7 intentionally leaves these systems unchanged except where specifically noted above:

- exact-model opening (never auto-hop on a tap);
- v2.15.3 orphaned pending-open self-healing;
- v2.15.4 active-phone queue lease/poll renewal;
- v2.15.2 bounded active-model look-ahead/refill (`N ahead` means current + N future mosaics);
- leaving/switching a model cancels that model's automatic look-ahead work;
- existing trustworthy mosaic reuse and exact hit-map validation;
- READY/index semantics and v2.14.9 conditional sidecar reindex behavior;
- Recu integration/capture/incremental scan;
- Keep Last session/cross-drive behavior;
- direct Range playback, automatic playback ladder and Preview diagnostics;
- Random, Avg Chunk and TS-only model ordering;
- Non-NSFW cleanup;
- destructive actions remain durable-PC-queue based.

## Validation

The modified source passed:

- Python `compileall`: PASS
- `node --check static/app.js`: PASS
- `node --check static/rapid.js`: PASS
- **28 / 28** `validation/validate_*.py` suites: PASS before packaging

New `validation/validate_v2_15_7_stabilization.py` includes:

- functional server stale-draft rejection by chunk signature;
- fake-ffprobe media named `_tail_1h35m...` returning an actual ~900-second duration and asserting preview uses ~900 seconds, not 5700;
- real headless Chromium file-level Shift select and Shift unselect ranges;
- real headless Chromium Frame Cut Shift select and Shift unselect ranges;
- real Clear-all Frame Cut control;
- loading Rapid JS and proving a new Review chunk clears old `frameDecisions` and both anchors while preserving Frame Cut granularity preference.

All inherited v2.13-v2.15.6 regression suites also pass.

## Files changed in v2.15.7
...
- `static/index.html`
- `static/service-worker.js`
- `mobile_self_test.py` (release marker only)

Validation:
- `validation/validate_v2_15_7_stabilization.py` (new)
- current-version assertions in inherited validations bumped to 2.15.7 / v2157
- `validation/validate_v2_15_1_shift_range.py` updated for symmetric range semantics

Documentation:
- `README.md`
- `CHANGELOG_v2_15_7.txt`
- `UPDATE_FROM_2_15_6.txt`
- `TEST_REPORT_v2_15_7.txt`
- `CTBRec_Mobile_Reviewer_v2_15_7_HANDOFF.md`

## Field validation checklist

After patching from v2.15.6:

1. Fully close/reopen desktop/iPhone PWA so shell `v2157` loads.
2. Review -> Frame Cut: select several frames, press Queue precise cuts & next. Confirm next mosaic has **zero selected frames**. Frame Cut itself should remain your active granularity preference if the next mosaic supports it.
3. Plain-click an item, Shift-click later -> inclusive range selects. Plain-click the latest anchor to turn it off, then Shift-click backward -> inclusive range clears.
4. Press the explicit Clear button in both Whole segments and Frame Cut modes.
5. Preview a ~15-minute media file with a much larger `_tail_...` tag. Confirm source timeline max matches the actual media duration.
6. Scrub near the actual end and confirm no black synthetic tail region exists.
7. Multi-file preview: force Direct/Remux failure if possible and confirm automatic ladder remains on the same file until true source end or terminal failure.
8. Confirm the configured look-ahead buffer continues to refill and leaving the model cancels future work as before.

## Known limitations / honest boundaries

- Actual media duration probing depends on ffprobe or FFmpeg being available when the browser itself cannot provide native source metadata. If neither can determine duration, fallback playback can still run sequentially but a trustworthy full-source scrub range cannot be invented.
- Very damaged/truncated media can remain unplayable even after Direct -> remux -> compatibility fallback; Preview diagnostics should be exported for those cases.
- In-flight FFmpeg mosaic work remains checkpoint-bounded rather than OS-process-hard-killed when a model is abandoned; no additional mosaics should start after cancellation.
- The GitHub ChatGPT connector currently visible in ordinary chat is read-only for this repository. User is setting up Codex/GitHub write workflow. **Do not email release artifacts going forward**; user explicitly replaced the former email-delivery requirement with Git/Codex as the intended source-of-truth workflow. Until write access is available, provide downloadable FULL SOURCE / PATCH / HANDOFF artifacts in chat.

## Exact next development steps

1. Field-test v2.15.7 selection reset and actual-duration preview on the specific files that exposed the bugs.
2. If any preview file still has incorrect duration, export the Preview diagnostics ZIP; inspect `ffprobe.json` plus duration events before changing heuristics.
3. Once Codex/GitHub write access is operational, initialize `fcd02/Chaturbate_Projects` as the umbrella repo and place this project under `projects/ctbrec-mobile-reviewer/`, with current `HANDOFF.md`, source, validation, and release history tracked there.
4. Maintain one canonical latest handoff in Git and archive prior handoffs under project docs/history.

---

# Prior v2.15.6 handoff (preserved verbatim for historical architecture/context)

# CTBRec Mobile Reviewer v2.15.6 — Complete Development Handoff

**Release state:** FINAL / GREEN. v2.15.5 baseline + source-timeline preview UX and automatic per-file playback ladder below. Complete v2.15.5-and-earlier handoff is appended after this authoritative section.

## User field report that created v2.15.6
The user confirmed v2.15.5 loads problematic recordings faster, but fallback playback still felt unlike a normal video player. Remux/Compatibility streams visibly restarted their timeline at 0:00 whenever the user scrubbed because each server-side source seek launches a new live FFmpeg stream. In multi-file preview, an early-ended failed stream could also trigger the generic `ended` handler and immediately advance to the next selected file, preventing the user from manually choosing Compatibility for the earlier files. The user requested normal source-timeline semantics and fully automatic selection of the fastest usable playback method.

## Root-cause findings
1. **Live FFmpeg pipes do not have a stable native source timeline.** v2.15.5 correctly restarted FFmpeg at a requested source timestamp, but the browser only knew about the newly started stream, so its native controls began again at zero. This was technically correct for the pipe and wrong for the user's mental model.
2. **`ended` was overloaded.** The client treated every media `ended` event as proof the source recording was finished. A remux/transcode process that died early can also cause `ended`, so a playback-method failure could incorrectly advance to the next selected file.
3. **Fallback selection should be an implementation detail.** Direct → remux → Compatibility existed, but the UI still exposed a workflow where the user could feel responsible for choosing Compatibility. The player should exhaust the ladder automatically for the current file before moving on.

## Implemented v2.15.6 changes
### Source-timeline fallback controls
- Direct original-file and native HLS playback retain ordinary browser controls because they are genuinely seekable.
- Remux/Compatibility playback hides the pipe's native controls and overlays a player-integrated source timeline directly at the bottom of the video.
- The source timeline always spans 0:00 → the full recording duration and displays absolute source time, even though the underlying FFmpeg stream begins internally at zero after a seek.
- Dragging/releasing the source timeline restarts the active fallback backend at that absolute source timestamp. The user never has to translate between stream-relative and source-relative time.
- Play/pause remains available inside the fallback player overlay. A manual **Force compatibility now** action remains as an optional override, but is no longer required for normal recovery.

### Automatic playback ladder
- The current file owns the playback attempt until its available methods are exhausted.
- Desktop/default order remains: original direct Range → fast zero-reencode remux → compatibility transcode. Native HLS remains the preferred seekable path where the browser supports it; HLS failures enter the same remux → Compatibility ladder.
- Startup/media/stall/seek watchdog failures advance methods automatically at the same absolute source position.
- Terminal failure stays on the current file and tells the user to export Preview diagnostics; it does not jump ahead.

### Premature-end guard for multi-file preview
- Added `previewReachedSourceEnd()`, which compares the absolute source playhead against the source duration with a small tolerance.
- `ended` advances to the next selected file only when the source is genuinely at its end.
- If a remux/transcode/direct path ends early, Reviewer records `premature_stream_end` and treats that as a failed method for the same file, triggering the next fallback stage.

## Non-regression boundary
v2.15.6 intentionally does not change queue lifecycle, active-model look-ahead, mosaic generation/reuse, READY/index semantics, Recu, Keep Last, model ordering (including TS-only size), Shift-click, destructive file actions, or whole-library background generation. v2.15.5 explicit media MIME mapping and Preview diagnostics remain intact.

## Validation
- Python compile: PASS.
- `static/app.js` syntax: PASS.
- Existing v2.13–v2.15.5 regression suites: PASS after only updating superseded preview assertions/version markers.
- New `validation/validate_v2_15_6_preview_autoladder.py`: PASS. It verifies integrated source-timeline controls, automatic direct/remux/Compatibility ladder markers, terminal same-file behavior, premature-end protection, and source-relative timing in real headless Chromium.
- Working-tree total: 27/27 validation suites PASS.

## Final packaging gate — PASS
- FULL SOURCE built from a clean extraction of the released v2.15.5 FULL SOURCE with only v2.15.6 code/docs/tests overlaid.
- Clean release-tree validation: Python/JS syntax PASS; all 27 validation suites PASS.
- Seeded v2.15.5→v2.15.6 PATCH overlay: 13/13 representative runtime/private sentinels remained byte-identical, including config, roots, action queue, catalog/READY/library/NSFW/model/Recu state/profile and the bundled NudeNet model.
- Overlay targeted v2.15.2–v2.15.6 regressions: PASS.
- PATCH contains no runtime/private state. FULL SOURCE starter PIN/secret/catalog/action/log state remains sanitized.

## Exact next steps for another AI agent
1. Preserve the source-timeline abstraction for remux/Compatibility; do not expose the backend pipe-relative 0:00 timeline again.
2. Preserve the automatic per-file ladder and premature-end guard. A failed method must never be mistaken for completion of the source recording.
3. If one recording still fails all methods, ask the user to reproduce it and export Preview diagnostics before changing codec/container logic.
4. Preserve v2.15.5 explicit MIME mapping, TS-only sort and robust Shift-click.
5. Preserve v2.15.4 active-view lease, v2.15.3 pending-open recovery and v2.15.2 bounded active-model look-ahead.
6. Every development response must include FULL SOURCE ZIP, PATCH ZIP, and current HANDOFF Markdown.

---

# Prior complete handoff (v2.15.5 and earlier)

# CTBRec Mobile Reviewer v2.15.5 — Complete Development Handoff

**Release state:** FINAL / GREEN. Candidate source and v2.15.4→v2.15.5 runtime-preservation overlay gates passed; final archive is revalidated after packaging. v2.15.4 baseline + preview reliability/diagnostics, TS-only model ordering, and Shift-click reliability changes below. Complete prior v2.15.4-and-earlier handoff is appended after this authoritative section.

## User field reports that created v2.15.5
The user reported that many `.mp4` recordings and then at least one `.ts` recording could not play directly from disk on the same PC that owns the attached drives. Falling back to Compatibility was slow, and dragging/scrubbing the video timeline caused long freezes before playback resumed. The user requested either a general fix or a useful exportable error log plus a TS-only size ordering mode. They also reported that the v2.15.1 Shift-click range-selection behavior still did not work in the real desktop browser.

## Root-cause findings
1. **TS Content-Type was OS-dependent.** `send_range_file()` used `mimetypes.guess_type()`. On some systems `.ts` is registered as Qt Linguist text rather than MPEG transport-stream video. Reviewer also sends `X-Content-Type-Options: nosniff`, so an incorrect text MIME can make an otherwise valid TS fail direct playback.
2. **Metadata success could disable failover too early.** The client cleared the preview watchdog in `loadedmetadata`. A source can expose duration/container metadata yet never decode a playable frame. That left the user staring at a stalled direct preview until manually choosing Compatibility.
3. **FFmpeg pipe fallbacks are sequential, not Range-seekable.** Remux and compatibility endpoints stream fragmented MP4 from FFmpeg stdout. Browser-native random seeking inside that live pipe cannot be made equivalent to seeking a static local file; dragging the timeline could therefore stall. The correct fast fallback is to restart FFmpeg with input-side `-ss` at the requested absolute source timestamp.
4. **Prior Shift-click depended too heavily on click.shiftKey.** Real desktop/PWA event paths can lose modifier state between pointerdown and click. The old static validation proved the code path existed but did not reproduce an actual browser modifier sequence.

## Implemented v2.15.5 changes
### Preview/direct-disk reliability
- Added explicit `video_content_type()` mapping for every supported video suffix. Exact `.ts` is always `video/mp2t`; MP4/M4V/MOV/MKV/AVI/WebM likewise receive stable video MIME types.
- Direct Range semantics remain unchanged and still support normal/open-ended/suffix requests. No source file mutation is introduced.
- The startup watchdog remains armed after `loadedmetadata` and clears only once `canplay`/`playing` proves decodable playback. Direct files that expose metadata but never decode now fail over automatically.
- `waiting`, `stalled`, and direct `seeking` states have bounded watchdogs. If a direct seek stalls, failover starts at the requested source timestamp instead of at zero.
- The existing direct → zero-reencode remux → compatibility transcode sequence remains, but stage transitions now preserve the desired absolute source position.

### Fast seeking for remux/compatibility fallback
- Added a visible **Fast seek (server-side)** range control when the preview is in remux or compatibility mode.
- A Fast-seek jump restarts the existing FFmpeg stream with input-side `-ss` at that source time. This avoids waiting for an impossible byte-range seek inside a live FFmpeg stdout pipe.
- Native `<video>` seek events in remux/compatibility mode are also converted into a short-debounced source-timestamp restart.
- Recu-derived initial offsets are no longer incorrectly applied twice after a remux/transcode stream has already started at that source timestamp.

### One-click preview diagnostics
- Every preview item receives a bounded in-memory diagnostic ID.
- Client records stage start, loadedmetadata, canplay/playing, media errors, waiting/stalled, seeks, failovers, and requested server-side seek restarts.
- Direct Range handler records requested ranges, status, byte interval, bytes sent, elapsed time, disconnects, and explicit MIME.
- Remux/transcode handler records start timestamp, mode, elapsed time, and process return status.
- **Export preview diagnostics** downloads a ZIP with `preview_summary.json`, `ffprobe.json`, a bounded `mobile_reviewer_log_tail.txt`, and a README. It never copies media/video bytes. It may contain model/file names and local filesystem paths.
- Diagnostic sessions are bounded in memory (80 recent previews; 250 events each).

### TS-only size model ordering
- Ordinary catalog scanning now computes `(total video bytes, exact .ts bytes)` in the same directory traversal; there is no second library crawl and no FFmpeg work.
- Catalog rows expose `ts_bytes`, `ts_size`, and `ts_stats_known`.
- New **TS-only size ↓** mode hides known-zero-TS models and ranks known TS models by exact `.ts` bytes descending while each card still shows the total model size.
- A v2.15.4 cached catalog has no per-extension subtotal. Such rows are marked unknown rather than falsely treated as zero. The UI tells the user to tap **Rescan once**; after that scan completes the filter/order is exact.

### Shift-click repair
- Shift state is now tracked from window keydown/keyup, pointerdown on the exact tile/file row, and click.shiftKey.
- `shiftRangeRequested()` accepts any of those reliable signals.
- Plain click behavior remains unchanged. Shift-click applies the current action/selection to the inclusive range from the immediately previous clicked item to the new target and keeps selections outside that range.
- File and Review Frame Cut anchors remain independent and reset on chunk/model change.
- Added real headless-Chromium regression: click file #2, hold Shift, click file #5 → exactly #2–#5 selected.

## Non-regression boundary
No intentional changes were made to v2.15.4 active-view lease semantics, v2.15.3 pending-open recovery, v2.15.2 current+N active-model look-ahead, mosaic generation, chunk definitions, READY/index reuse, destructive action semantics, Recu, Keep Last, whole-library background generation, or model-open behavior.

## Validation
Validation gates pass: Python compile; `app.js`, `rapid.js`, `service-worker.js` syntax; all 26 `validation/*.py` suites, including the new v2.15.5 preview/TS/diagnostics/real-Chromium Shift-click suite. The v2.15.4→v2.15.5 PATCH overlay preserved 13 representative runtime/private sentinels byte-for-byte and passed v2.15.2–v2.15.5 targeted regressions. Full-source starter state is sanitized. See `TEST_REPORT_v2_15_5.txt`.

## Final packaging gate — PASS
- Candidate clean source: 26/26 validation suites PASS.
- Real Chromium Shift-click interaction: PASS.
- PATCH protected-state inventory: PASS; no runtime/private state included.
- Seeded v2.15.4→v2.15.5 overlay: 13/13 representative runtime/private sentinels byte-identical.
- Overlay v2.15.2, v2.15.3, v2.15.4, and v2.15.5 targeted regressions: PASS.
- Full-source starter PIN/secret/catalog/action/log sanitation: PASS.
- Final ZIP clean-extraction revalidation is performed after packaging; if future agents rebuild either ZIP, rerun that exact gate.

## Packaging / runtime-state safety
PATCH must contain code/static/docs/tests only. It must exclude active/private runtime state including `mobile_config.json`, `recording_roots.txt`, `mobile_action_queue.json`, catalog/READY/library/NSFW state, model metadata, Recu session/profile, logs, generated mosaics/media, and model binaries. FULL SOURCE must contain sanitized starter state only.

## Exact next steps for another AI agent
1. Preserve explicit video MIME mappings; do not return to OS-dependent `.ts` MIME detection.
2. Preserve the distinction between direct static Range seeking and live FFmpeg pipe fallback. If fallback seek still feels slow, optimize restart latency; do not pretend the pipe itself is byte-range-seekable.
3. If a specific recording still fails, ask the user to reproduce it and export the new Preview diagnostics ZIP before changing codec/container logic.
4. TS-only stats are exact only after a catalog scan that writes `ts_bytes`; never silently interpret old rows as zero TS.
5. Preserve pointerdown/global Shift tracking and the real Chromium range-selection regression.
6. Preserve all v2.15.4 queue lifecycle and v2.15.2 look-ahead invariants.
7. Every development response must include FULL SOURCE ZIP, PATCH ZIP, and current HANDOFF Markdown.

---

# Prior complete handoff (v2.15.4 and earlier)

# CTBRec Mobile Reviewer v2.15.4 — Complete Development Handoff

**Release state:** FINAL / GREEN. v2.15.3 baseline + active visible-queue lease correction below. Complete prior v2.15.3-and-earlier handoff is appended after this authoritative section.

## User field report that created v2.15.4
The user supplied an iPhone screenshot while visibly inside a model queue. The mosaic panel said: **“Stopped mosaic generation because this model is no longer open on the phone.”** This contradicted the actual UI state and prevented the requested v2.15.2 automatic look-ahead behavior.

## Root cause
v2.15.3 still treated the 12-second JavaScript sorting heartbeat as the sole authoritative evidence that the review queue remained open. The lease lasts 45 seconds. iOS can delay/throttle JavaScript timers even while the PWA is visibly open. At the same time, the phone can continue making queue-specific `/api/queues/<id>/status` requests roughly every 2.2 seconds. Those successful requests did **not** renew the lease.

When the lease expired, `request_interactive_mosaic()` and `schedule_prefetch()` could cancel work because `sort_session_active()` returned false, producing the exact screenshot message even though the phone continued polling that same queue. Once current mosaic status became `cancelled`, the client no longer considered it a running state and could stop status polling, leaving the cancellation visible.

## Implemented v2.15.4 changes
1. **Durable per-queue phone-open state.** Queues now carry `phone_open`. Opening/activating a queue sets it true. Explicit leave/switch sets it false before cancelling hidden work.
2. **Queue-specific poll lease renewal.** `_touch_sort_queue_from_poll()` lets authenticated `/current` and `/status` calls renew/reclaim the active lease only for a queue whose `phone_open` flag is still true and only when no different queue is currently active. This makes successful queue polling stronger evidence than an iOS timer alone.
3. **Explicit leave remains authoritative even after timer expiry.** `mark_sort_session_inactive()` closes all queues still carrying `phone_open=true`, not merely `active_sort_queue_id`. This matters because an expired timer may already have cleared `active_sort_queue_id` before the explicit leave POST arrives.
4. **Switching models closes stale open flags.** `mark_sort_session_active(queue_id=...)` deactivates any other queue still marked phone-open, including queues whose timer lease expired earlier.
5. **Self-recovery after false cancellation.** If a legitimate visible-queue `/current` or `/status` poll finds the current mosaic or bounded look-ahead state cancelled specifically because the model was considered “no longer open,” it restarts that missing work.
6. **No product-behavior expansion.** v2.15.2 bounded current+N-ahead generation remains unchanged; v2.15.3 pending-model self-heal remains unchanged; Shift-click, Random/Avg Chunk model sorting, playback, READY/index semantics, Recu, Keep Last and whole-library behavior are unchanged.
7. **Versioning:** server `CTBRecMobile/2.15.4`; PWA cache `ctbrec-shell-v2154`; static assets `?v=2154`.

## New regression coverage
`validation/validate_v2_15_4_active_view_lease.py` verifies:
- a queue explicitly opened on the phone can reclaim an expired heartbeat lease from a queue-specific poll;
- explicit leave closes the durable phone-open flag and stale polls cannot resurrect it;
- explicit leave still closes the queue even if timer expiry already cleared `active_sort_queue_id`;
- switching from q1 to q2 closes q1 and opens only q2;
- the exact screenshot state (current + prefetch falsely cancelled as “no longer open”) self-recovers from a legitimate status poll;
- a truly closed queue does not restart.

All 25 validation suites passed in the working tree before packaging.

## Final packaging gate — PASS
- Candidate FULL SOURCE extracted cleanly; Python compilation and JS syntax checks passed.
- All 25 validation suites passed from the clean extracted FULL SOURCE bytes.
- PATCH inventory contains no active runtime/private state, generated media, Recu profile/session, logs, or model binaries.
- v2.15.3→v2.15.4 PATCH overlay preserved 13 representative seeded runtime/private sentinels byte-for-byte.
- Overlayed build passed v2.15.2 look-ahead, v2.15.3 pending-open recovery, and v2.15.4 active-view lease regressions.
- FULL SOURCE starter config is sanitized with blank access PIN/secret; starter catalog/action state is empty and log is zero bytes.

## Packaging / safety rules
The v2.15.3→v2.15.4 PATCH must contain only changed code/static/docs/tests and must not include active/private runtime state such as `mobile_config.json`, `recording_roots.txt`, action/catalog/READY/library/NSFW state, model metadata, Recu session/profile, logs, generated mosaics/media, or model binaries. FULL SOURCE must use sanitized starter runtime files.

## Exact next steps for another AI agent
1. Preserve `_touch_sort_queue_from_poll()` semantics: queue-specific polling may renew only a queue the phone explicitly owns via `phone_open`.
2. Never make timer heartbeat the sole source of truth again on iOS.
3. Preserve explicit leave/switch as authoritative and prevent stale poll resurrection.
4. Preserve v2.15.2 bounded look-ahead: current + configured N future mosaics, active model only.
5. If another false cancellation appears, collect `mobile_reviewer.log` and diagnostics around the queue ID before changing queue lifecycle semantics.
6. Every development response must include FULL SOURCE ZIP, PATCH ZIP, and current HANDOFF Markdown.

---

# Prior complete handoff (v2.15.3 and earlier)
